...
3. **細項展開層級鐵則(重點)**:
   ...
   - `合計`/`小計`/`淨額` 這類加總列**不是**投資標的,不要放進 `rows`——那是
     ...
     金額」擋下來,等於白白製造一個假警報。
   - **但「沖減列」要抄進 `rows`,不要跟加總列一起丟掉**:
     `減：備抵損失`／`備抵損失`／`累計減損`／`評價調整`／`金融資產評價調整`
     這一類**不是**投資標的,但它們是「逐項毛額 → 資產負債表帳面金額」中間
     少掉的那一段,下游要用它才對得上 BS。名稱**照表上原樣抄**(含「減：」),
     金額**照表上原樣抄**(表上印負號就填負數,印正數就填正數,不要自己改符號)。
     ⚠️ 實測漏掉的後果:富邦 202302 AC 附註 p42 印著「減：備抵損失 628,162」,
     沒抄進來 → 逐列加總 797,475,709 比 BS 的 796,847,547 多 628,162,
     這一格就永遠對不上錨。5 份 AC 都栽在同一件事上。
   - **`printed_subtotal` 要填「扣完之後的最終合計」,不是中間的小計**:
     附註常見的排版是
         短期票券      436,402,937
         政府公債        9,634,615
         …
         小計          501,313,630     ← **不要填這個**
         減：備抵損失    ( 46,844 )
                     $ 501,266,786     ← **要填這個**(它就是資產負債表上的數)
     ⚠️ 最後那一行**常常沒有標籤**,只印一個 `$ 金額`,不要因為它沒有「合計」
     兩個字就跳過它去抓上面的「小計」。判準是:`printed_subtotal` 必須等於
     `bs_anchor`(資產負債表上該科目的金額),兩者對不上就是抓錯行了。
     多段式的表(債務工具段/權益工具段各有自己的小計與沖減列)同理 ——
     填最後那個總合計,不是任何一段的小計或淨額。
     **一律照抄,不要自己加減。**
4. 每個數字附出處頁碼。
