...
global.fetch = async (url) => {
  if (url.includes("/api/doc?doc=202404_兆豐_個體")) return { json: async () => JSON.parse(fs.readFileSync("out.json", "utf8")), ok: true };
...
const patched = workbench.replace("boot();", "boot().then(async () => { try { await viewDoc('202404_兆豐_個體', { reload: true }); console.log('viewDoc successful!'); } catch(e) { console.error('viewDoc failed:', e); } });");
try {
  eval(patched);
} catch(e) {
