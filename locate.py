...
EXPAND_TRUTH = [
    ...
    ("202402_玉山_個體", "OCI", 23),
    ("202502_玉山_個體", "OCI", 24),
    ("202304_富邦_個體", "Trading", 38),   # 同頁多段
    ("202401_中信_合併", "OCI", 15),       # 跨頁:債務小計在前一頁
    ("202501_中信_合併", "OCI", 17),
    ("202502_中信_合併", "OCI", 17),
    ("202502_中信_個體", "OCI", 16),
]
...
class Located:
    ...
    @property
    def basis(self):
        """報表口徑(個體/合併),從封面判 —— 見 `basis_of` 與 `cover_text`。"""
        return basis_of(cover_text(self.texts))
...
#: 一頁要有幾個字(去空白後)才算「讀得到內容」。見 `cover_text()`。
MIN_COVER_CHARS = 100


def cover_text(texts):
    """判口徑要看的那一頁 —— **第一頁讀得到內容的頁面**,不是固定第 0 頁。

    為什麼不是 `texts[0]`:有些財報的前十幾頁(封面、會計師查核報告)是
    **掃描影像,文字層是空的**。原本只看第 0 頁,這種檔一律回 `UNKNOWN`,
    而 `UNKNOWN` 在下游等同「這份文件不存在」(`report.cell_of` 直接回 None)。
    於是「封面是掃描圖」跟「真的判不出口徑」塌成同一個結果 —— 鐵律 9 講的
    就是這件事。實測 `202002_富邦_個體`:前 14 頁零文字,p14 起明寫
    「台北富邦商業銀行股份有限公司**個體財務報表**附註」,而它有 3 格 21 列
    已抄資料,原本卻因為判不出口徑而永遠進不了發布網格。

    **這不是針對版型的規則**(鐵律 2):它沒有寫死頁碼、沒有認任何一家的
    排版,只是把「封面」定義成「第一頁看得到字的」,對文字層完整的檔
    (p0 就有字)行為完全不變。

    `MIN_COVER_CHARS` 的敏感度實測(2026-08-12,全部 91 份):
    門檻 **1 ~ 300 的結果逐份完全相同**(救回 2 份、既有判定 0 份被改),
    400 開始有 14 份被改掉(跳過了真正的封面 —— 封面字數本來就少)。
    取 100 是平坦區中段,離 400 那道崖夠遠;結論不依賴這個數字取多少。
    """
    for t in texts or ():
        if len(re.sub(r"\s", "", t or "")) >= MIN_COVER_CHARS:
            return t
    return ""
